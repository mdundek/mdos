...
The full instance of MDos is packed with extensions and features that go beyond the basic application deployment needs. It allows you to abstract away concepts such as SSO, certificate management, and multi-tenancy on one single cluster along with advanced RBAC features, a private secured registry and more. This is useful if you start from scratch and you are managing your Kubernetes cluster yourself versus using a managed Kubernetes cluster such as EKS, GKE, Openshift and so on.  

---

### 2. And/or as an application deployment framework only (onto your own cluster)

<img align="left" width="450" style="margin-right:50px" src="https://github.com/mdundek/mdos/blob/main/mdos-docs/infra/mkdocs/docs/img/mdosyaml.png?raw=true"> 
...
With the MDos framework & CLI, this is what an application deployment looks like for YAML config file just like the one on the left:
...
4. Provide you with feedback on how the deployment is going
...
That said, its complexity is often a deterrent, and leads to badly designed deployment scenarios, regularly exposing security threats as well as the miss-usage of certain cluster-based capabilities, leading to under-utilization of its capabilities.  
...
**MDos is an attempt to solve some of those complex issues for developers and companies, letting them focus on developing applications and not on how to get them running securely on Kubernetes.**
...
Therefore, build your applications using higher-level artifacts that will translate to lower-level Kubernetes resource definitions based on Kubernetes best practices.  

* Scaffold a new `application` workspace
* Scaffold an application `component` inside your mdos `application`
