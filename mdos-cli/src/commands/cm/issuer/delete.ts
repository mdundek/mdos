    ...
    public async run(): Promise<void> {
        ...
        let responses = await inquirer.prompt([{
            ...
            choices: allIssuers.map((issuer:any) => {
                return {
                    name: issuer.kind == "Issuer" ? `${issuer.metadata.namespace}/${issuer.metadata.name} (${issuer.kind})` : `${issuer.metadata.name} (${issuer.kind})`,
                    value: issuer
                }
            }),
        }, {
        ...
        if(responses.confirm) {
            ...
            try {
                if(responses.issuer.kind == "ClusterIssuer") {
                    ...
                    await this.api(`kube/${responses.issuer.metadata.name}?target=cm-issuer&namespace=${responses.issuer.metadata.namespace}`, 'delete')
                }
