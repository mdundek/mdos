...
class Keycloak {
    ...
    async createRealm(realm) {
        ...
        if(!responseRealm.data.find(r => r.realm == realm)) {
            await axios.post(`https://keycloak.${this.rootDomain}/admin/realms`, {
                "id": realm,
                "realm": realm,
                "rememberMe": true, 
                "enabled": true
    ...
    async createClient(realm, clientId) {
        ...
        await axios.post(`https://keycloak.${this.rootDomain}/admin/realms/${realm}/clients`, {
            "clientId": clientId,
            "rootUrl": "",
            "baseUrl": "",
    ...
    async createClientRole(realm, clientUuid, name) {
        ...
        if(!clientInst)
            throw new NotFound("Client UUID not found");

        

console.log(`${clientInst.clientId}_${name}`);
        await axios.post(`https://keycloak.${this.rootDomain}/admin/realms/${realm}/clients/${clientUuid}/roles`, {
    ...
    async createUser(realm, userName, password, email) {
        ...
        await axios.post(`https://keycloak.${this.rootDomain}/admin/realms/${realm}/users`, {
            "username": userName,
            "enabled": true,
            "totp": false,
            "emailVerified": true,
        ...
        await axios.put(`https://keycloak.${this.rootDomain}/admin/realms/mdos/users/${userUuid}/reset-password`, {
            "type": "password",
            "value": password,
            "temporary":false
        }, {
    ...
    async createClientRoleBindingForUser(realm, clientUuid, userUuid, roleUuid, roleName) {
        let accessToken = await this._getAccessToken();
        await axios.post(`https://keycloak.${this.rootDomain}/admin/realms/${realm}/users/${userUuid}/role-mappings/clients/${clientUuid}`, [
            {
                "id": roleUuid,
                "name": roleName
            }
